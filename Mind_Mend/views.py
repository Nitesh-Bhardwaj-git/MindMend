...
from .forms import SignUpForm, MoodEntryForm, ForumPostForm, ForumReplyForm, CounsellorBookingForm, ContactForm
...
def forum_list(request):
    posts = ForumPost.objects.all().annotate(reply_count=Count('forumreply')).order_by('-created_at')
    paginator = Paginator(posts, 10)
    page = request.GET.get('page', 1)
    posts = paginator.get_page(page)
    return render(request, 'Mind_Mend/forum_list.html', {'posts': posts})
...
def my_bookings(request):
    bookings = CounsellorBooking.objects.filter(user=request.user).order_by('-date', '-time_slot')
